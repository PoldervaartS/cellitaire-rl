...
class ModelBuilder:
    ...
    def build_model(self):
        ...
        # If embedding: each slot is an integer (card id) and we use an
        # Embedding layer.
        if card_rep == "one_hot":
            ...
            board_emb = Embedding(
                input_dim=53,
                output_dim=emb_dim,
                name="board_embedding")(board_input)
            # Process with RNN.
            board_rnn = GRU(
                cfg.get(
                    "board_rnn_units",
        ...
        if card_rep == "one_hot":
            ...
            # For embedding, top card is an integer, and the remaining count is
            # a scalar.
        ...
        if card_rep == "one_hot":
            ...
            foundation_flat = Flatten(
                name="foundation_flatten")(foundation_input)
        elif card_rep == "embedding":
            # For embedding, foundation input is 4 integers.
            foundation_input = Input(
                shape=(
                    4,
                ),
                dtype='int32',
                name="foundation_input")
            # Embed each integer.
            emb_dim = cfg.get("embedding_dim", 10)
            foundation_emb = Embedding(
                input_dim=53,
                output_dim=emb_dim,
                name="foundation_embedding")(foundation_input)
            foundation_flat = Flatten(
                name="foundation_flatten")(foundation_emb)

        # --- COMBINE INPUTS ---
        # Concatenate board, stock, and foundation features.
        combined = Concatenate(name="combined_features")(
            [board_features, stock_hidden, foundation_flat])

        # Two hidden dense layers.
        hidden1 = Dense(
            cfg["hidden_layer_sizes"][0],
            activation="relu",
            name="hidden1")(combined)
        hidden2 = Dense(
            cfg["hidden_layer_sizes"][1],
            activation="relu",
        ...
        # For example, using MSE loss for value regression.
